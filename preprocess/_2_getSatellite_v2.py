...
def saveTiff(r, g, b, gt, filePath):
    ...
    dset_output = None
    # print(f"{city_name} {index} Image Saved")
...
def main(left, top, right, bottom, zoom, filePath, style='s', server="Google China"):
    ...
    urls_group = [urls[i:i + math.ceil(len(urls) / count)] for i in
                  range(0, len(urls), math.ceil(len(urls) / count))]
...
if __name__ == '__main__':
    ...
    for city in range(2):
        ...
        print(city_name)
        file = f"../data/{city_name}/Boundary/union.csv"
        df = pd.read_csv(file)
        for index, row in df.iterrows():
            if os.path.exists(f'../data/{city_name}/Satellite/{index}.tif'):
                continue
            geom = wkt.loads(row[2])
            # print(geom.bounds)
            # (-74.17850477726755, 40.68732670189682, -74.16949576825853, 40.69633571090583)
            main(geom.bounds[0], geom.bounds[3], geom.bounds[2], geom.bounds[1], 20,
                 f'../data/{city_name}/Satellite/{index}.tif', server="Google", style='s')
            time.sleep(1)
